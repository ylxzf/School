...
//import packs.onebilion.CheaperCharBuffer;
...
public class Generate1BRC3 {
    ...
    public static void main(final String[] args) throws Exception {
        ...
        if (args.length != 1) {
            System.out.println("Usage: Generate1BRC <number of records to create>");
            System.exit(1);
        }
        ...
        final int strideSize = 50_000_000;
        final int outer = size / strideSize;
        final int remainder = size - (outer * strideSize);
        ...
        try (final BufferedWriter bw = new BufferedWriter(new FileWriter(FILE))) {
            for (int i = 0; i < outer; i++) {
                ...
                System.out.println("Wrote %,d measurements in %s ms".formatted((i + 1) * strideSize,
            ...
            bw.flush();
            final long millisDuration = System.currentTimeMillis() - start;
            System.out.println(
                    "Created file with %,d measurements in %s ms -> %s"
                            .formatted(size, millisDuration, getDurationBreakdown(millisDuration)));
        }
    }

    private static void produce(final BufferedWriter bw, final List<WeatherStation> stations, final int count)
            throws IOException {
        final int stationCount = stations.size();

        for (int i = 0; i < count; i++) {
            bw.write(stations.get(rnd.nextInt(stationCount)).measurement());
        }
    }
...
class WeatherStation {
    private final WeatherStationAbstract ws;

    public WeatherStation(final String id, final double meanTemperature) {
        ws = new WSOwnTableDouble(id, meanTemperature);
    }

    @Override
    public String toString() {
        // return "WeatherStation [ws=" + ws + "]";
        return ws.toString();
    }
    ...
    public double getMeanTemperature() {
        return this.ws.getMeanTemperature();
    }

    public String measurement() {
        return ws.measurement();
    }

}

abstract class WeatherStationAbstract {
    protected final static Random rnd = new Random();
    protected final String id;
    protected final double meanTemperature;

    /* form: "<City ID>;<temerature %3.1f>\n" */
    /* +-20 Celsius Degree, Continuous uniform distribution */
    abstract public String measurement();

    public WeatherStationAbstract(final String id, final double meanTemperature) {
        this.id = id;
    ...
    public final String toString() {
        return "[" + getClass().getName() + "] Statiton: " + getId() + ", mean temperature: " +
                String.format(Locale.ENGLISH, "%3.1f", getMeanTemperature());
    }
    ...
    public final double getMeanTemperature() {
        return this.meanTemperature;
    }
}

final class WSFirstNaive extends WeatherStationAbstract {
    public WSFirstNaive(final String id, final double meanTemperature) {
        super(id, meanTemperature);
    }

    @Override
    public String measurement() {
        // return getId() + String.format(Locale.ENGLISH, ";%3.1f\n", (-20 + 40 *
        // rnd.nextDouble() + getMeanTemperature()));
        return String.format(Locale.ENGLISH, "%s;%3.1f\n", getId(),
                (-20 + 40 * rnd.nextDouble() + getMeanTemperature()));
    }
}

final class WSSecondNaive extends WeatherStationAbstract {
    private final static DecimalFormat df = new DecimalFormat("#.0");

    public WSSecondNaive(final String id, final double meanTemperature) {
        super(id, meanTemperature);
    }

    @Override
    public String measurement() {
        return getId() + ";" + df.format((-20 + 40 * rnd.nextDouble() + getMeanTemperature())) + "\n";
    }
}

final class WSSecondNaive2 extends WeatherStationAbstract {
    private final static DecimalFormat df = new DecimalFormat("#.0");

    public WSSecondNaive2(final String id, final double meanTemperature) {
        super(id + ";", meanTemperature);
    }
...
final class WSThirdNaive extends WeatherStationAbstract {
    private final static DecimalFormat df = new DecimalFormat("#.0");
    private final static StringBuffer sb = new StringBuffer(100);

    public WSThirdNaive(final String id, final double meanTemperature) {
        super(id + ";", meanTemperature);
    }
    ...
    public String measurement() {
        sb.setLength(0);
        sb.append(getId()).append(df.format((-20 + 40 * rnd.nextDouble() + getMeanTemperature()))).append("\n");
        return sb.toString();
    }
}

final class WSOwnTableDouble extends WeatherStationAbstract {
    // private final static StringBuffer sb = new StringBuffer(100);
    private final String[] table = new String[401]; /* +-20 */

    public WSOwnTableDouble(final String id, final double meanTemperature) {
        super(id + ";", meanTemperature);

        // double arithmetics
        final DecimalFormat df = new DecimalFormat("#.0");
        int i=0;
        for (double d = (getMeanTemperature() - 20); d <= (getMeanTemperature() + 20); d += .1) {
            table[i++] = df.format(d) + "\n";
        }
    }
    ...
    @Override
    public String measurement() {
        return getId() + table[rnd.nextInt(401)];
    }
}

// (-20 + 40 * rnd.nextDouble() + getMeanTemperature())

final class WSSharedTable extends WeatherStationAbstract {
    private static final ArrayList<String> sharedTable = new ArrayList<>();
